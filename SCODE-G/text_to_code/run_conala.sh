...
path_2_data="/local/rizwan/workspace/projects/RaProLanG/data/plbart/conala/${LANG}_retrievd_from_with_ref_top_${TOP_K}_mask_rate_0"
...
if [[ "$USE_PLBART" = true ]]; then
    ...
    MODEL_DIR_NAME="plbart-conala-${LANG}-with-top-${TOP_K}-retrived-from-with-ref-no-mask"
else
    MODEL_DIR_NAME="transformer-conala-${LANG}-with-top-${TOP_K}-retrived-from-no-ref-no-mask"
fi
