...
LANG=${2:-"python"}
USE_PLBART=${3:-true}
WITH_OR_WITHOUT_REF=${4:-without} #with or no are same use without
TOP_K=${4:-5}
task=translation_from_pretrained_bart


path_2_data="/local/rizwan/workspace/projects/RaProLanG/data/plbart/csnet_with_comments/${LANG}_${WITH_OR_WITHOUT_REF}_ref_top_${TOP_K}"
...
if [[ "$USE_PLBART" = true ]]; then
    ...
    MODEL_DIR_NAME="plbart-codexglue-csnet-${LANG}-comments-top-${TOP_K}-${WITH_OR_WITHOUT_REF}"
else
