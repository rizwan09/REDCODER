...
TARGET=en_XX
top_k=${2:-30}
WITH_WITHOUT_SUFFIX=${3:-with} #with or without
GPU=${4:-2,3}
...
PATH_2_DATA=/local/rizwan/workspace/projects/RaProLanG/data/plbart/csnet_code_text_with_codes_${SOURCE}/${WITH_WITHOUT_SUFFIX}_ref_top_${top_k}/data-bin
#PATH_2_DATA=/local/rizwan/workspace/projects/RaProLanG/data/plbart/retrieve_csenet_parallel_only_csnet_code_text_with_codes_${SOURCE}/${WITH_WITHOUT_SUFFIX}_ref_top_${top_k}/data-bin
SPM_MODEL=/local/wasiahmad/codebart/sentencepiece.bpe.model


echo "Source: $SOURCE Target: $TARGET"

SAVE_DIR=/local/rizwan/workspace/projects/RaProLanG/csnet_code_text_with_codes_${SOURCE}/${SOURCE}_${TARGET}_${WITH_WITHOUT_SUFFIX}_${top_k}
mkdir -p ${SAVE_DIR}
...
function fine_tune () {
	...
	fairseq-train $PATH_2_DATA $USER_DIR \
		...
		--bpe 'sentencepiece' --sentencepiece-model $SPM_MODEL \
  		...
  		--batch-size 16 --update-freq 8 --max-epoch 15 \
