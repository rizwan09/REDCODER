...
PATH_2_DATA=/local/rizwan/workspace/projects/RaProLanG/data/plbart/bm25/csnet_code_text_${SOURCE}/${WITH_WITHOUT_SUFFIX}_ref_top_${top_k}/data-bin
SPM_MODEL=/local/wasiahmad/codebart/sentencepiece.bpe.model


echo "Source: $SOURCE Target: $TARGET"

SAVE_DIR=/local/rizwan/workspace/projects/RaProLanG/csnet_bm25_code_text_${SOURCE}/${SOURCE}_${TARGET}_${WITH_WITHOUT_SUFFIX}_${top_k}
mkdir -p ${SAVE_DIR}
...
function fine_tune () {
	...
	fairseq-train $PATH_2_DATA $USER_DIR \
		...
		--bpe 'sentencepiece' --sentencepiece-model $SPM_MODEL \
  		...
  		--batch-size 8 --update-freq 16 --max-epoch 15 \
