...
LANG=${1:-java}
top_k=${2:-5}
WITH_OR_WITHOUT_REF=${2:-with}
#WITH_OR_WITHOUT_REF=${2:-no}

SPMDIR=/local/wasiahmad/codebart
RETDIR=/local/rizwan/workspace/projects/RaProLang/retrieval/bm25
SAVE_DIR="/local/rizwan/workspace/projects/RaProLanG/data/plbart/concode/bm25top_${top_k}_${WITH_OR_WITHOUT_REF}"
mkdir -p $SAVE_DIR
...
for split in train valid test; do
    python process.py \
        --retrieved_code_file $RETDIR/concode.${split}_bm25.json \
        --split $split \
        --top_k $top_k \
        --out_dir $SAVE_DIR \
