...
from dpr.utils.data_utils_with_comments import ShardedDataIterator, read_data_from_json_files, Tensorizer
...
class BiEncoderTrainer(object):
    ...
    def get_data_iterator(self, path: str, batch_size: int, shuffle=True,
                          shuffle_seed: int = 0,
                          offset: int = 0, upsample_rates: list = None, debug = False, valid=False) -> ShardedDataIterator:
        ...
        data = read_data_from_json_files(data_files, upsample_rates, \
                                         ...
                                         dataset=self.args.dataset, valid=valid)

        # filter those without positive ctx
        data = [r for r in data if len(r['positive_ctxs']) > 0 and r['question'] ]
    ...
    def validate_nll(self) -> float:
        ...
        args = self.args
        self.biencoder.eval()
        data_iterator = self.get_data_iterator(args.dev_file, args.dev_batch_size, shuffle=False, valid=True)
